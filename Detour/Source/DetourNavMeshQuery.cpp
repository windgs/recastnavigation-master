...
static const float H_SCALE = 0.999f; // Search heuristic scale.//搜索启发式量表
										 ...
										 dtPolyRef* randomRef, float* randomPt) const
{
	...
	for (int i = 0; i < m_nav->getMaxTiles(); i++)
	{
		...
		// 使用Reservoi采样选择随机图块
		const float area = 1.0f; // Could be tile area too.//可能也是瓦片区
		tsum += area;
		const float u = frand();
		if (u*tsum <= area)
			tile = t;
													 ...
													 dtPolyRef* randomRef, float* randomPt) const
{
	...
	while (!m_openList->empty())
	{
		...
		// 获取poly和tile。API输入已经被检查过，跳过检查内部数据
...
///
///将使用批多边形调用查询。传递给查询的多边形具有与传递给此函数的中心和半范围重叠的边界框。多次调用dtpolyquery:：process函数，直到处理完所有重叠的多边形。
dtStatus dtNavMeshQuery::queryPolygons(const float* center, const float* halfExtents,
									   const dtQueryFilter* filter, dtPolyQuery* query) const
{
	dtAssert(m_nav);
											...
											const dtQueryFilter* filter, const unsigned int options)
{
	...
	if (options & DT_FINDPATH_ANY_ANGLE)
	{
		...
		// 限制为字符半径的几倍会得到很好的结果。它不敏感，所以从第一个图块计算就足够了
	...
	*straightPathCount = 0;
	...
	if (pathSize > 1)
	{
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (i+1 < pathSize)
			{
				unsigned char fromType; // fromType is ignored.// 已忽略FromType
				...
				if (dtStatusFailed(getPortalPoints(path[i], path[i+1], left, right, fromType, toType)))
				{
					...
					// 无法获取入口点，实际上这意味着path[i+1]是无效的多边形。将端点夹在path[i]上，并返回到目前为止的路径
					
					if (dtStatusFailed(closestPointOnPolyBoundary(path[i], endPos, closestEndPos)))
					{
						// This should only happen when the first polygon is invalid.
						// 只有当第一个多边形无效时才会发生这种情况
						return DT_FAILURE | DT_INVALID_PARAM;
					}
...
///
/// HITPOS不使用高度详细数据进行调整。如果半径内没有墙，hittist将等于搜索半径。在这种情况下，@p hitpos和@p hitnormal的值是未定义的。如果@p hittist是一个非常小的数字，则正常值将变得不可预测
