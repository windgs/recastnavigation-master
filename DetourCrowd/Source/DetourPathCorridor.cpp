...
这是用于规划走廊内局部移动的功能。一个或多个角可以
检测以计划移动。它基本上执行与dtnavmeshquery:：findstraightpath相同的功能
...
由于内部优化，返回的最大角数将为（@p maxcorners-1）
例如：如果缓冲区的大小可以容纳10个角，那么函数将永远不会返回超过9个角。
因此，如果需要10个角，缓冲器的大小应该适合11个角

If the target is within range, it will be the last corner and have a polygon reference id of zero.
如果目标在范围内，它将是最后一个角，并且多边形参考ID为零
*/
...
预期的使用情况是，所需位置将“靠近”当前走廊。什么叫“近”
取决于局部多边形密度、查询搜索半范围等。

如果所需位置不在导航网格上，则结果位置将与所需位置不同，
或者使用本地搜索无法访问。
*/
...
-移动被限制在导航网格的表面。
-走廊自动调整（短或长），以保持有效。
-新目标将位于调整后的走廊的最后一个多边形中。

预期的使用案例是期望的目标将“靠近”当前走廊。什么是“近”取决于局部多边形密度、查询搜索半范围等。

如果所需目标不在导航网格上，或者使用本地搜索无法到达，则生成的目标将与所需目标不同
*/
