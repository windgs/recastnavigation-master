...
/// 群组代理在路径中向前看的最大角落数。此值用于调整群组代理转角缓冲区的大小。由于群组管理器的行为，实际可用转角数将比此数少一个
/// @ingroup crowd
...
{
	DT_CROWDAGENT_STATE_INVALID,		///< The agent is not in a valid state. ///<代理处于无效状态
	DT_CROWDAGENT_STATE_WALKING,		///< The agent is traversing a normal navigation mesh polygon.///代理正在遍历普通导航网格多边形
	DT_CROWDAGENT_STATE_OFFMESH,		///< The agent is traversing an off-mesh connection.///代理正在遍历非网格连接
};
...
struct dtCrowdAgentParams
{
	...
	/// 定义在考虑转向行为之前，碰撞元素必须有多近。[限制：>0]
	float collisionQueryRange;

	float pathOptimizationRange;		///< The path visibility optimization range. [Limit: > 0]

	/// How aggresive the agent manager should be at avoiding collisions with this agent. [Limit: >= 0]
	/// 代理管理器在避免与此代理冲突时应具有多大的攻击性。[极限：>＝0 ]
	float separationWeight;

	/// Flags that impact steering behavior. (See: #UpdateFlags)
	/// 影响转向行为的标志。（请参见：UpdateFlags）
...
struct dtCrowdAgent
{
	...
	/// 角色正在遍历的网格多边形的类型。（见：众包状态）
	unsigned char state;

	/// True if the agent has valid path (targetState == DT_CROWDAGENT_TARGET_VALID) and the path does not lead to the requested position, else false.
	/// 如果代理具有有效路径（targetstate==dt_crowdagent_target_valid），并且路径未指向请求的位置，则为true，否则为false
	bool partial;
	...
	float npos[3];		///< The current agent position. [(x, y, z)] ///< 当前代理位置。[（x，y，z）]
	float disp[3];		///< A temporary value used to accumulate agent displacement during iterative collision resolution. [(x, y, z)] ///< 用于在迭代碰撞解决过程中累积代理位移的临时值。[（x，y，z）]
	float dvel[3];		///< The desired velocity of the agent. Based on the current path, calculated from scratch each frame. [(x, y, z)] ///< 代理所需的速度。基于当前路径，从每个帧开始计算。[（x，y，z）]
	float nvel[3];		///< The desired velocity adjusted by obstacle avoidance, calculated from scratch each frame. [(x, y, z)]	///< 由障碍物回避调整的期望速度，从每个帧的划痕计算。[（x，y，z）]
	float vel[3];		///< The actual velocity of the agent. The change from nvel -> vel is constrained by max acceleration. [(x, y, z)] ///< 代理的实际速度。这一改变从nvel >速度受最大加速度的限制。[（x，y，z）]
	...
	/// 代理的本地路径走廊角。（直线路径）。[（x，y，z）*n角]
	float cornerVerts[DT_CROWDAGENT_MAX_CORNERS*3];

	/// The local path corridor corner flags. (See: #dtStraightPathFlags) [(flags) * #ncorners]
	/// 本地路径走廊角标志。（请参见：dtStraightPathFlags）[（Flags）*ncorners]
	unsigned char cornerFlags[DT_CROWDAGENT_MAX_CORNERS];

	/// The reference id of the polygon being entered at the corner. [(polyRef) * #ncorners]
	/// 在角点输入的多边形的参考ID。[（polyref）*入侵者]
	...
	unsigned char targetState;			///< State of the movement request. ///< 移动请求的状态
	dtPolyRef targetRef;				///< Target polyref of the movement request. ///< 移动请求的目标polyref
	float targetPos[3];					///< Target position of the movement request (or velocity in case of DT_CROWDAGENT_TARGET_VELOCITY). ///< 移动请求的目标位置（或dt_crowdagent_target_velocity情况下的速度）。
	dtPathQueueRef targetPathqRef;		///< Path finder ref. ///< 路径查找器参考
	bool targetReplan;					///< Flag indicating that the current path is being replanned. ///< 指示正在重新连接当前路径的标志。
	float targetReplanTime;				/// <Time since the agent's target was replanned. ///< 代理的目标重新连接后的时间
};
