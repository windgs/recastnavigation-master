...
作者：fananchong
来源：CSDN
原文：https://blog.csdn.net/u013272009/article/details/80086833
...
Sample* createSolo() { return new Sample_SoloMesh(); }							//创建单独mesh
Sample* createTile() { return new Sample_TileMesh(); }							//创建瓦片mesh
Sample* createTempObstacle() { return new Sample_TempObstacles(); }				//创建临时障碍物
...
int main(int /*argc*/, char** /*argv*/)
{
	...
	//多点采样抗锯齿运算 是一种全屏的抗锯齿运算，多点采样默认是禁用的，但是你可以设定SDL_GL_MULTISAMPLEBUFFERS的值为1 并且设定 SDL_GL_MULTISAMPLESAMPLES 的值大于零（典型值为2和4）来启用多点采样.
	// 4x MSAA.
	...
	*/
	...
	float scrollZoom = 0;//	滚动缩放
	bool rotate = false;	
	bool movedDuringRotate = false;//移动向下旋转
	float rayStart[3];
	float rayEnd[3];
	bool mouseOverMenu = false;//鼠标菜单
	
	bool showMenu = !presentationMode;//		演示模式
	bool showLog = false;
	bool showTools = true;
	...
	while(!done)
	{
		...
		if (showLog && showMenu)
		{
			...
			if (imguiBeginScrollArea("Log", 250 + 20, 10, width - 300 - 250, 600, &logScroll))
			//add end
				mouseOverMenu = true;
