...
void Sample::handleCommonSettings()
{
	...
	*/
	//Rasterization：用于设定体素的尺寸。
	imguiLabel("Rasterization-体素的尺寸");
	//cellSize：较小的值能够产生更加精确接近原始集合图形的网格，减少在生成导航网格过程中生成多边形产生较大偏离度的问题，但是会消耗更加多的处理时间和内存占用。
	imguiSlider("Cell Size-设置体素voxel在X-Z-plane)上的大小", &m_cellSize, 0.1f, 1.0f, 0.01f);
	//cellHeight：较低的值并不会对内存消耗产生有显著影响，较低的值虽然能够使网格贴合原始图形，但是如果是凹凸不平的地形，较低的值可能会造成邻接的网格之间产生断裂，
	//导致本来应该连在一起的网格造成分离。设置较高时，可以看到网格和原始图形间距较大。
	imguiSlider("Cell Height-设置体素voxel在Y-plane)上的大小", &m_cellHeight, 0.1f, 1.0f, 0.01f);
	...
	imguiLabel("Agent-角色");
	//Height: 角色的身高。
	imguiSlider("Height-身高", &m_agentHeight, 0.1f, 5.0f, 0.1f);
	//Radius：角色的半径。
	imguiSlider("Radius-半径", &m_agentRadius, 0.0f, 5.0f, 0.1f);
	//Max Climb：描述成生活中爬楼梯时，每个台阶的高度。
	imguiSlider("Max Climb-爬坡高度", &m_agentMaxClimb, 0.1f, 5.0f, 0.1f);
	//Max Slope：描述成生活中爬坡的坡度，以角度值进行设置。
	imguiSlider("Max Slope-可行走倾斜角度", &m_agentMaxSlope, 0.0f, 90.0f, 1.0f);
	
	imguiSeparator();
	//区域
	imguiLabel("Region-区域");
	//最小区域尺寸
	imguiSlider("Min Region Size-最小区域尺寸", &m_regionMinSize, 0.0f, 150.0f, 1.0f);
	//合并区域尺寸
	imguiSlider("Merged Region Size-合并区域尺寸", &m_regionMergeSize, 0.0f, 150.0f, 1.0f);

	imguiSeparator();
	//高度域分离方式
	imguiLabel("Partitioning-高度域分离方式");
	//分水岭算法
	//Watershed ：分水岭算法，最经典、效果最好，但处理比较慢，一般用于离线处理。
	if (imguiCheck("Watershed-分水岭算法", m_partitionType == SAMPLE_PARTITION_WATERSHED))
		m_partitionType = SAMPLE_PARTITION_WATERSHED;
	//Monotone算法
	//Monotone ：最快且可以保证生成的是不重叠、没有洞的 Region，但是生成的 Region 可能会又细又长，效果不好。
	if (imguiCheck("Monotone-Monotone算法", m_partitionType == SAMPLE_PARTITION_MONOTONE))
		m_partitionType = SAMPLE_PARTITION_MONOTONE;
	//分层算法
	//Layer：速度、效果都介乎分水岭算法和 Monotone partioning 之间，比较依赖于初始数据。
	if (imguiCheck("Layers-分层算法", m_partitionType == SAMPLE_PARTITION_LAYERS))
	...
	imguiSeparator();
	//多边形设置
	imguiLabel("Polygonization-多边形设置");
	//最大三角边长度
	imguiSlider("Max Edge Length-最大三角边长度", &m_edgeMaxLen, 0.0f, 50.0f, 1.0f);
	//最大边容错
	imguiSlider("Max Edge Error-最大边容错", &m_edgeMaxError, 0.1f, 3.0f, 0.1f);
	//每个三角形顶点数
	imguiSlider("Verts Per Poly-每个三角形顶点数", &m_vertsPerPoly, 3.0f, 12.0f, 1.0f);		

	imguiSeparator();
	imguiLabel("Detail Mesh-细节网格");
	//采样距离
	imguiSlider("Sample Distance-采样距离", &m_detailSampleDist, 0.0f, 16.0f, 1.0f);
	//最大采样容错
	imguiSlider("Max Sample Error-最大采样容错", &m_detailSampleMaxError, 0.0f, 16.0f, 1.0f);
