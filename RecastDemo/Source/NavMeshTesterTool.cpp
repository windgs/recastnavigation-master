	...
	if (imguiButton("Make Random Points"))
	{
		...
		for (int i = 0; i < MAX_RAND_POINTS; i++)
		{
			...
			if (dtStatusSucceed(status))
			{
				...
				//add by huyf 2019.02.21
				{
					m_sample->getContext()->log(RC_LOG_PROGRESS, "sum:%d   ref:[%d]   m_nrandPoints:(%f %f %f)",
						m_nrandPoints, ref, pt[0], pt[1], pt[2]);
				}
				//add end
				m_nrandPoints++;
			}
...
void NavMeshTesterTool::recalc()
{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
	{
		...
		if (m_sposSet && m_eposSet && m_startRef && m_endRef)
		{
			...
			//add by huyf 2019.02.21
			{
				m_sample->getContext()->log(RC_LOG_PROGRESS, "\npi  %f %f %f  %f %f %f  0x%x 0x%x\n",
					m_spos[0], m_spos[1], m_spos[2], m_epos[0], m_epos[1], m_epos[2],
					m_filter.getIncludeFlags(), m_filter.getExcludeFlags());
			}
			//add end
			...
			m_nsmoothPath = 0;
			//add by huyf
			m_sample->getContext()->log(RC_LOG_PROGRESS, "------------------start--------------------------------------\n\n");
			//add end
			if (m_npolys)
			{
				// Iterate over the path to find smooth path on the detail mesh surface.
				...
				static const float STEP_SIZE = 0.5f;
				static const float SLOP = 0.01f;	//距离目标最小距离
				...
				// Move towards target a small advancement at a time until target reached or
				// when ran out of memory to store the path.
				// 每次向目标前进一小步，直到达到目标，当存储路径的内存耗尽的时候
				//add by huyf
				m_sample->getContext()->log(RC_LOG_PROGRESS, "------------------start pathfinding:[%d]--------------------------------------\n\n", m_npolys);
				int nSum = npolys;
				//add end
				while (npolys && m_nsmoothPath < MAX_SMOOTH)
				{
					...
					//add by huyf 2019.02.21
					{
						m_sample->getContext()->log(RC_LOG_PROGRESS, "step:%d   iter:(%f %f %f)   steer:(%f %f %f)   epos:(%f %f %f)   m_polys[%d]:ref[%d]", 
							nSum- npolys, iterPos[0], iterPos[1], iterPos[2], steerPos[0], steerPos[1], steerPos[2], m_epos[0], m_epos[1], m_epos[2], nSum - npolys, m_polys[nSum - npolys]);
					}
					//add end
					...
					// Move-移动
					float result[3];
					dtPolyRef visited[16];		//固定16 size of Store result
					int nvisited = 0;
					// 沿着表面移动
	...
	else if (m_toolMode == TOOLMODE_PATHFIND_STRAIGHT)
	{
		if (m_sposSet && m_eposSet && m_startRef && m_endRef)
		{
			...
			if (m_npolys)
			{
				...
				//add by huyf 2019.02.26
				int pos = 0;
				dtPolyRef nCurRef;
				float currpos[3];
				for (int i = 0; i < m_nstraightPath * 3; )
				{
					currpos[0] = m_straightPath[i++];
					currpos[1] = m_straightPath[i++];
					currpos[2] = m_straightPath[i++];
					pos++;
					m_navQuery->findNearestPoly(currpos, m_polyPickExt, &m_filter, &nCurRef, 0);

					m_sample->getContext()->log(RC_LOG_PROGRESS, "step:[%d]   ref:[%d]   turnpos:(%f %f %f)\n",
						pos, nCurRef, currpos[0], currpos[1], currpos[2]);
				}				
				//add end
			}
		}
	...
	else if (m_toolMode == TOOLMODE_RAYCAST)
	{
		...
		if (m_sposSet && m_eposSet && m_startRef)
		{
			...
			//add by huyf 2019.02.21
			{
				m_sample->getContext()->log(RC_LOG_PROGRESS, "ref:[%d] spos:(%f %f %f)  epos:(%f %f %f)   hitPos:(%f %f %f)  hitNormal:(%f %f %f） IncFlags:0x%x ExcFlags:0x%x\n",
					m_startRef, m_spos[0], m_spos[1], m_spos[2], m_epos[0], m_epos[1], m_epos[2], m_hitPos[0], m_hitPos[1], m_hitPos[2], m_hitNormal[0], m_hitNormal[1], m_hitNormal[2],
					m_filter.getIncludeFlags(), m_filter.getExcludeFlags());
			}
			//add end
		}
...
#ifdef DUMP_REQS			
			{
				m_sample->getContext()->log(RC_LOG_PROGRESS, "dw  s_ref:[%d]   spos:(%f %f %f)   maxRadius:%f  distance:%f  hitPos:(%f %f %f)  hitNormal:(%f %f %f)  IncFlags:0x%x ExcFlags:0x%x\n",
					m_startRef, m_spos[0], m_spos[1], m_spos[2], 100.0f, m_distanceToWall, m_hitPos[0], m_hitPos[1], m_hitPos[2], m_hitNormal[0], m_hitNormal[1], m_hitNormal[2],
					m_filter.getIncludeFlags(), m_filter.getExcludeFlags());
			}
#endif
			//add end
	...
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_SHAPE)
	{
		if (m_sposSet && m_startRef && m_eposSet)
		{
			...
			//add by huyf 2019.02.21
			{
				m_sample->getContext()->log(RC_LOG_PROGRESS, "fpp  spos:(%f %f %f)  %f %f %f  %f %f %f  %f %f %f  0x%x 0x%x\n",
					m_queryPoly[0], m_queryPoly[1], m_queryPoly[2],
					m_queryPoly[3], m_queryPoly[4], m_queryPoly[5],
					m_queryPoly[6], m_queryPoly[7], m_queryPoly[8],
					m_queryPoly[9], m_queryPoly[10], m_queryPoly[11],
	...
	else if (m_toolMode == TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD)
	{
		if (m_sposSet && m_startRef)
		{
			...
			//add by huyf 2019.02.21
			{
				m_sample->getContext()->log(RC_LOG_PROGRESS, "fln  spos:(%f %f %f)  nbhr:%f  IncF:0x%x ExcF:0x%x\n",
					m_spos[0], m_spos[1], m_spos[2], m_neighbourhoodRadius,
					m_filter.getIncludeFlags(), m_filter.getExcludeFlags());
			}
			//add end
