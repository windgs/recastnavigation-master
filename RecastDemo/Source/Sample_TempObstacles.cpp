...
public:
	...
	virtual void handleMenu()
	{
		...
		imguiSeparator();
		if (imguiCheck("Draw Areas-区域", m_drawType == DRAWDETAIL_AREAS))
			m_drawType = DRAWDETAIL_AREAS;
		if (imguiCheck("Draw Regions-出生点", m_drawType == DRAWDETAIL_REGIONS))
			m_drawType = DRAWDETAIL_REGIONS;
		if (imguiCheck("Draw Contours-轮廓", m_drawType == DRAWDETAIL_CONTOURS))
			m_drawType = DRAWDETAIL_CONTOURS;
		if (imguiCheck("Draw Mesh-网格", m_drawType == DRAWDETAIL_MESH))
...
public:
	...
	virtual void handleMenu()
	{
		...
		if (imguiButton("Remove All-删除所有"))
...
void Sample_TempObstacles::handleSettings()
{
	...
	if (imguiButton("Save-保存"))
	{
		saveAll("all_tiles_tilecache.bin");
	}

	if (imguiButton("Load-加载"))
	{
		dtFreeNavMesh(m_navMesh);
		dtFreeTileCache(m_tileCache);
...
void Sample_TempObstacles::handleTools()
{
	...
	if (imguiCheck("Test Navmesh-测试网格", type == TOOL_NAVMESH_TESTER))
	{
		setTool(new NavMeshTesterTool);
	}
	if (imguiCheck("Highlight Tile Cache-突出瓷砖缓存", type == TOOL_TILE_HIGHLIGHT))
	{
		setTool(new TempObstacleHilightTool);
	}
	if (imguiCheck("Create Temp Obstacles-创建临时的障碍", type == TOOL_TEMP_OBSTACLE))
	{
		setTool(new TempObstacleCreateTool);
	}
	if (imguiCheck("Create Off-Mesh Links-创建Off-Mesh链接", type == TOOL_OFFMESH_CONNECTION))
	{
		setTool(new OffMeshConnectionTool);
	}
	if (imguiCheck("Create Convex Volumes-创建凸卷", type == TOOL_CONVEX_VOLUME))
	{
		setTool(new ConvexVolumeTool);
	}
	if (imguiCheck("Create Crowds-创建人群", type == TOOL_CROWD))
	{
...
void Sample_TempObstacles::handleDebugMode()
{
	...
	imguiLabel("Draw");
	if (imguiCheck("Input Mesh-输入网格", m_drawMode == DRAWMODE_MESH, valid[DRAWMODE_MESH]))
		m_drawMode = DRAWMODE_MESH;
	if (imguiCheck("Navmesh-导航网格", m_drawMode == DRAWMODE_NAVMESH, valid[DRAWMODE_NAVMESH]))
		m_drawMode = DRAWMODE_NAVMESH;
...
void Sample_TempObstacles::addTempObstacle(const float* pos)
{
	...
	dtVcopy(p, pos);
	p[1] -= 0.5f;
	//障碍物 radius:0.1f height:2.0f result
	...
	e[1] = 8.0f;
	e[2] = -5.80f;
	m_tileCache->addBoxObstacle(s, e, 0);
	//add end
}
